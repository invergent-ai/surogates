...
Single integration point for the harness loop.  
...
Usage::
    ...
    # Optionally: manager.add_provider(external_provider)
    await manager.initialize_all()
    prompt_block = manager.build_system_prompt()
    context = await manager.prefetch_all(user_message)
"""
...
class MemoryManager:
    ...
    async def handle_tool_call(
        ...
        try:
            result = await provider.handle_tool_call(tool_name, args)
        except Exception as e:
            logger.error(
        ...
        if action in ("add", "replace") and result:
            ...
            await self.on_memory_write(action, target, content)

...
Only one external provider runs at a time to prevent tool schema bloat and
conflicting memory backends.

Registration:

1. Built-in: BuiltinMemoryProvider -- always present, not removable.
2. Plugins: activated by ``memory.provider`` config.
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        - ``hermes_home`` (str): The active home directory path.
    ...
    def save_config(self, values: dict[str, Any], hermes_home: str) -> None:
        ...
        Called after collecting user inputs during setup.
        ``values`` contains only non-secret fields (secrets go to ``.env``).
        ``hermes_home`` is the active home directory path.
        """

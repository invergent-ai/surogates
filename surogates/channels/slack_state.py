"""Redis-backed Slack adapter state.

The shared Slack adapter runs as one (or more) pods serving many workspaces.
...
sets.  Keys are agent-scoped and TTL-bounded (default 7 days) which also
replaces the old manual size-trim of the in-memory structures.
"""
...
class SlackAdapterState:
    """Durable per-agent Slack routing/mention state."""

    def __init__(
        self, redis: Any, *, agent_id: str, ttl_seconds: int = _DEFAULT_TTL,
    ) -> None:
        self._redis = redis
    ...
    def _session_key(self, session_key: str) -> str:
        return f"mate:slack:sessionmap:{self._agent_id}:{session_key}"

    def _mentioned_key(self, thread_ts: str) -> str:
        return f"mate:slack:mentioned:{self._agent_id}:{thread_ts}"

    def _botmsg_key(self, ts: str) -> str:
        return f"mate:slack:botmsg:{self._agent_id}:{ts}"

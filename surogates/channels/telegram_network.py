...
class TelegramFallbackTransport(httpx.AsyncBaseTransport):
    ...
    def __init__(self, fallback_ips: Iterable[str], **transport_kwargs: object):
        self._fallback_ips = [
            ip for ip in dict.fromkeys(_normalize_fallback_ips(fallback_ips))
        ]
                ...
                if ip is not None and self._sticky_ip != ip:
                    async with self._sticky_lock:
                        if self._sticky_ip != ip:
                            self._sticky_ip = ip
                            logger.warning(
                                "[Telegram] Primary api.telegram.org path unreachable; "
                                "using sticky fallback IP %s",
                                ip,
                            )
                return response
...
def _normalize_fallback_ips(values: Iterable[str]) -> list[str]:
    """Validate and normalise an iterable of IP address strings."""
    normalized: list[str] = []
    for value in values:
        raw = str(value).strip()
        if not raw:
...
def parse_fallback_ip_env(value: str | None) -> list[str]:
    """Parse a comma-separated string of fallback IPs."""
    if not value:
        return []
...
def _rewrite_request_for_ip(request: httpx.Request, ip: str) -> httpx.Request:
    """Rewrite an httpx Request to connect to *ip* while preserving the Host header."""
...
def _is_retryable_connect_error(exc: Exception) -> bool:
    """Return True for connect-phase errors worth retrying on a different IP."""

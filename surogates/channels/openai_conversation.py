...
#: The ``synthetic`` value stamped on a replayed transcript entry.  Mirrors
#: :data:`surogates.session.events.SEED_SYNTHETIC_MARKER`; imported rather
#: than re-spelled would drag a session import into this pure module, so the
#: parity is asserted by the test suite instead.
SEED_MARKER = "seed"
...
def normalise_explicit_id(raw: str | None) -> str | None:
    """The accepted conversation id, or ``None`` when there is none.

    Public so callers decide "pinned or not" from the SAME answer the key is
    derived from. A blank or over-long header falls back to derived keying;
    a caller that treated the raw header as pinned would then reconcile as
    pinned against a derived key and silently lose the isolation the header
    exists to provide.
    """
    return _explicit_id(raw)
...
def session_user_turns(events: Iterable[Any]) -> list[str]:
    ...
    Two kinds of ``user.message`` carry a ``synthetic`` marker, and they must
    be treated in opposite ways:

    * ``seed`` — a turn WE wrote when creating or forking this session,
      replaying what the caller said elsewhere.  It **is** the caller's
      history, so it counts.  Skipping it would leave a forked session unable
      to match the caller ever again: every following request would re-fork
      and re-seed the whole conversation, losing the memory, workspace and
      browser the session exists to carry, and growing quadratically.
    * anything else (``outcome_kickoff``, ``ambient_tick``, ``mission_nudge``,
      ``channel_history_backfill`` …) — generated by the agent for itself.
      The caller never sent it, so counting it would make every following
      request look like a rewrite.
    """
    turns: list[str] = []
    for event in events:
...
class ReconcileAction(str, Enum):
    ...
    FORK = "fork"
    #: This exact turn is already running in the session — a retry of a
    #: request whose answer the client never received.  Attach to the turn in
    #: flight instead of asking the agent the same question twice.
    ATTACH = "attach"
...
def reconcile(
    ...
    *prompt* is the turn about to run, supplied so a retry of a turn already
    in flight can attach to it rather than duplicate it.
    ...
    if prompt is not None and existing == [*caller, normalise_text(prompt)]:
        # The session already holds this exact turn. An OpenAI SDK retries on
        # timeout with an identical body, and the slow turns most likely to
        # time out are exactly the ones a duplicate would cost the most —
        # double work, double billing, two divergent answers.
        return Reconciliation(action=ReconcileAction.ATTACH, seed_turns=[])

...
This module provides the glue:
    - ``TokenStorage``: persists tokens/client-info to disk so they
      survive across process restarts.
    - Callback server: ephemeral localhost HTTP server to capture the OAuth
      redirect with the authorization code.
    - ``build_oauth_auth()``: entry point called by ``client.py`` that wires
    ...
    mcp_servers:
      my_server:
        ...
        oauth:                                  # all fields optional
          ...
          client_name: "My Custom Client"       # default: "Surogates Agent"
          token_dir: "/path/to/tokens"          # override token storage dir
"""
...
# Default token directory.  Can be overridden per-server via config.
_DEFAULT_TOKEN_DIR: Path | None = None
...
def _get_token_dir(override: str | None = None) -> Path:
    """Return the directory for MCP OAuth token files.

    Uses the override if provided, otherwise falls back to
    ``~/.surogates/mcp-tokens/``.
    """
    if override:
        return Path(override)
    if _DEFAULT_TOKEN_DIR is not None:
        return _DEFAULT_TOKEN_DIR
    return Path(os.environ.get("SUROGATES_HOME", str(Path.home() / ".surogates"))) / "mcp-tokens"


def set_default_token_dir(path: str | Path) -> None:
    """Set the default token storage directory for OAuth tokens.

    Called by the platform integrator to configure where tokens are stored.
    """
    global _DEFAULT_TOKEN_DIR
    _DEFAULT_TOKEN_DIR = Path(path)
...
class TokenStorage:
    ...
    File layout::
        ...
        <token_dir>/<server_name>.json         -- tokens
        <token_dir>/<server_name>.client.json   -- client info
    """
    ...
    def _client_info_path(self) -> Path:
        return _get_token_dir(self._token_dir_override) / f"{self._server_name}.client.json"
    ...
    class _Handler(BaseHTTPRequestHandler):
        def do_GET(self) -> None:  # noqa: N802
            ...
            body = (
                ...
                "<p>You can close this tab and return to Surogates.</p></body></html>"
            ) if code else (
...
def remove_oauth_tokens(server_name: str, token_dir: str | None = None) -> None:
    """Delete stored OAuth tokens and client info for a server."""
    storage = TokenStorage(server_name, token_dir=token_dir)
    storage.remove()
...
def build_oauth_auth(
    ...
    Called from ``client.py`` when a server has ``auth: oauth`` in config.

    Args:
        server_name: Server key in mcp_servers config (used for storage).
        server_url: MCP server endpoint URL.
        oauth_config: Optional dict from the ``oauth:`` block in config.
    ...
    # --- Client metadata ---
    client_name = cfg.get("client_name", "Surogates Agent")
    scope = cfg.get("scope")

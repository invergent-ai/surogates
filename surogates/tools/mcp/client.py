...
discovers their tools, and registers them into the Surogates ToolRegistry
so the agent can call them like any built-in tool.

Configuration is passed via constructor params (not config.yaml).
...
class SamplingHandler:
    ...
    async def __call__(self, context, params):
        ...
        Note: The ``call_llm`` function must be supplied by the platform
        integrator via ``set_sampling_llm_caller()``.
        """
        # Rate limit
        ...
        # Get auxiliary LLM client via module-level callback
        if _sampling_llm_caller is None:
            self.metrics["errors"] += 1
            return self._error(
                "Sampling LLM caller not configured -- call "
                "set_sampling_llm_caller() before enabling sampling"
            )
...
# ---------------------------------------------------------------------------
# Sampling LLM caller -- set by platform integrator
# ---------------------------------------------------------------------------
...
class MCPServerTask:
    ...
    async def _refresh_tools(self):
        ...
        async with self._refresh_lock:
            ...
            # 2. Deregister old tools from the registry
            for prefixed_name in self._registered_tool_names:
                self._registry.deregister(prefixed_name)

            # 3. Re-register with fresh tool list
            self._tools = new_mcp_tools
            self._registered_tool_names = _register_server_tools(
                self.name, self, self._config, self._registry,
            )
            ...
            # 4. Notify callback if set
            if self._on_tools_changed is not None:
                try:
                    self._on_tools_changed(self.name, self._registered_tool_names)
                except Exception:
                    logger.exception(
                        "on_tools_changed callback failed for '%s'", self.name,
                    )
    ...
    async def _run_http(self, config: dict):
        ...
        if self._auth_type == "oauth":
            try:
                from surogates.tools.mcp.oauth import build_oauth_auth
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # --- AGT MCPResponseScanner (OWASP ASI02) ---
            # Scan tool response for prompt injection markers,
            # credential leaks, and exfiltration URLs before returning
            # to the LLM context.
            from agent_os.mcp_response_scanner import MCPResponseScanner

            scan = MCPResponseScanner().scan_response(
                text_result, tool_name=f"{server_name}/{tool_name}"
            )
            if not scan.is_safe:
                threat_descs = "; ".join(t.description for t in scan.threats)
                logger.warning(
                    "MCP response scan blocked %s/%s: %s",
                    server_name, tool_name, threat_descs,
                )
                return json.dumps({
                    "error": (
                        f"MCP response blocked by security scan: {threat_descs}"
                    )
                })
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Converts hyphens to underscores, and replaces any other character
    outside ``[A-Za-z0-9_]`` with ``_`` so generated tool names are
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Surogates registry schema format.
...
def _build_utility_schemas(server_name: str) -> List[dict]:
    """Build schemas for the MCP utility tools (resources and prompts).

    Returns a list of dicts with keys: schema, handler_key.
    """
...
def _register_server_tools(
    ...
    Handles include/exclude filtering and utility tools.

    Args:
        name: The server name.
        server: The connected MCPServerTask.
        config: The server config dict.
    ...
    for mcp_tool in server._tools:
        if not _should_register(mcp_tool.name):
            ...
            continue
        schema_dict = _convert_mcp_schema(name, mcp_tool)
        tool_name_prefixed = schema_dict["name"]

        # Guard against collisions with existing tools.
        if registry.has(tool_name_prefixed):
            logger.warning(
                "MCP server '%s': tool '%s' (-> '%s') collides with "
                "existing tool -- skipping to preserve existing",
    ...
    for entry in _select_utility_schemas(name, server, config):
        schema_dict = entry["schema"]
        handler_key = entry["handler_key"]
        handler = _handler_factories[handler_key](name, server.tool_timeout)
        util_name = schema_dict["name"]

        # Same collision guard for utility tools.
        if registry.has(util_name):
            logger.warning(
                "MCP server '%s': utility tool '%s' collides with "
                "existing tool -- skipping to preserve existing",
                name, util_name,
            )
            continue
...
async def _discover_and_register_server(
    ...
    registered_names = _register_server_tools(name, server, config, registry)
...
def discover_mcp_tools(
    ...
    """Connect to MCP servers, discover tools, and register them.

    This is the main entry point for the Surogates platform.  Config is
    passed via the ``servers`` dict (not read from config.yaml).
    ...
    Args:
        servers: Mapping of ``{server_name: server_config}``.
            Each config can contain either ``command``/``args``/``env`` for
            stdio transport or ``url``/``headers`` for HTTP transport, plus
            optional ``timeout``, ``connect_timeout``, ``auth``, and
            ``sampling`` overrides.  ``${ENV_VAR}`` placeholders in string
            values are resolved from ``os.environ``.
        registry: The ToolRegistry instance to register tools into.
    ...
    # Interpolate env vars in config values
    ...
    async def _discover_one(name: str, cfg: dict) -> List[str]:
        ...
        return await _discover_and_register_server(name, cfg, registry)
    ...
    # Log a summary so callers get visibility into what was registered.
    with _lock:

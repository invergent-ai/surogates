...
This module loads a user-managed website blocklist from the Surogates config
directory and optional shared list files. It is intentionally lightweight so
web/browser tools can enforce URL policy without pulling in the heavier config
stack.
...
def _get_default_config_dir() -> Path:
    """Return the default Surogates configuration directory."""
    return Path.home() / ".surogates"


def _get_default_config_path() -> Path:
    return _get_default_config_dir() / "config.yaml"
...
def load_website_blocklist(config_path: Optional[Path] = None) -> Dict[str, Any]:
    ...
    for shared_file in raw_shared_files:
        ...
        if not path.is_absolute():
            path = (_get_default_config_dir() / path).resolve()

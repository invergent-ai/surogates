...
Architecture:
    ~/.surogates/checkpoints/{sha256(abs_dir)[:16]}/   — shadow git repo
        HEAD, refs/, objects/                           — standard git internals
        SUROGATES_WORKDIR                              — original dir path
...
def _get_checkpoint_base() -> Path:
    """Return the base directory for checkpoint shadow repos.

    Configurable via the ``SUROGATES_CHECKPOINT_DIR`` environment variable.
    Defaults to ``~/.surogates/checkpoints``.
    """
    env_val = os.getenv("SUROGATES_CHECKPOINT_DIR")
    if env_val:
        return Path(env_val).expanduser().resolve()
    return Path.home() / ".surogates" / "checkpoints"
...
_GIT_TIMEOUT: int = max(10, min(60, int(os.getenv("SUROGATES_CHECKPOINT_TIMEOUT", "30"))))
...
def _init_shadow_repo(shadow_repo: Path, working_dir: str) -> Optional[str]:
    ...
    _run_git(["config", "user.email", "surogates@local"], shadow_repo, working_dir)
    _run_git(["config", "user.name", "Surogates Checkpoint"], shadow_repo, working_dir)
    ...
    (shadow_repo / "SUROGATES_WORKDIR").write_text(
...
class CheckpointManager:
    ...
    def latest_hash(self, working_dir: str) -> Optional[str]:
        """Return the commit hash of the most recent checkpoint, or None."""
        abs_dir = str(Path(working_dir).resolve())

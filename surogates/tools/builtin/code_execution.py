"""Builtin execute_code tool -- Programmatic Tool Calling (PTC).

Lets the LLM write a Python script that calls Surogates tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (UDS transport, local execution only):

1. Parent generates a ``surogates_tools.py`` stub module with UDS RPC
   functions for each allowed sandbox tool.
...
# ---------------------------------------------------------------------------
# surogates_tools.py code generator
# ---------------------------------------------------------------------------
...
def _generate_surogates_tools_module(enabled_tools: list[str]) -> str:
    """Build the source code for the ``surogates_tools.py`` stub module.

    Only tools in both :data:`SANDBOX_ALLOWED_TOOLS` and *enabled_tools*
    get stubs.  Transport is always UDS (local execution).
    """
...
"""Auto-generated Surogates tools RPC stubs."""
...
def _connect():
    ...
    if _sock is None:
        ...
        _sock.connect(os.environ["SUROGATES_RPC_SOCKET"])
...
def _tool_error(message: str) -> str:
    """Return a JSON-encoded error string for RPC responses."""
...
def _rpc_server_loop(
    ...
    **dispatch_kwargs: Any,
    ...
    Runs in a daemon thread.  Tool dispatch goes through
    :meth:`ToolRegistry.dispatch` on the caller's event loop via
        ...
        while True:
            ...
            while b"\n" in buf:
                ...
                try:
                    ...
                    resp = _tool_error(f"Invalid RPC request: {exc}")
                    conn.sendall((resp + "\n").encode())
                    continue
                ...
                # Dispatch through the Surogates tool registry.
                # We run the async dispatch on the caller's event loop
...
async def _execute_code_handler(
    arguments: dict[str, Any],
    **kwargs: Any,
) -> str:
    """Run a Python script in a sandboxed child process with RPC access
    to a subset of Surogates tools.

    Uses the local UDS (Unix domain socket) transport.  The child process
    communicates tool calls back to the parent via the socket; tool
    dispatch goes through :meth:`ToolRegistry.dispatch`.

    Parameters
    ----------
    arguments:
        Must contain a ``"code"`` key with the Python source to execute.
    **kwargs:
        Forwarded from the harness.  Must include ``"tools"``
        (:class:`ToolRegistry`).  May include ``"session_id"``,
        ``"tenant"``, etc.
    ...
    registry: ToolRegistry | None = kwargs.get("tools")
    if registry is None:
        return _tool_error(
            "execute_code requires a ToolRegistry but none was provided."
        )

    # Determine which tools the sandbox can call
    enabled_tools: list[str] | None = kwargs.get("enabled_tools")
    ...
    # Collect kwargs to pass through to tool dispatch (session_id, tenant, etc.)
    dispatch_kwargs: dict[str, Any] = {}
    for key in ("session_id", "tenant", "task_id"):
        if key in kwargs:
            dispatch_kwargs[key] = kwargs[key]

    # Grab the running event loop for cross-thread dispatch
    event_loop = asyncio.get_running_loop()

    # --- Set up temp directory with surogates_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="surogates_sandbox_")
    sock_path = os.path.join(
        tempfile.gettempdir(),
        f"surogates_rpc_{uuid.uuid4().hex}.sock",
    )
    ...
    try:
        # Write the auto-generated surogates_tools module
        tools_src = _generate_surogates_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "surogates_tools.py"), "w") as f:
            f.write(tools_src)
        ...
        rpc_thread = threading.Thread(
            ...
            args=(
                server_sock, registry, tool_call_log,
        ...
        # --- Spawn child process ---
        # Build a minimal environment for the child.  We intentionally
        # exclude API keys and tokens to prevent credential exfiltration
        # from LLM-generated scripts.  The child accesses tools via RPC,
        # not direct API.
        _SAFE_ENV_PREFIXES = (
        ...
        # --- Poll loop: watch for exit and timeout ---
        ...
        stdout_reader = threading.Thread(
            target=_drain_stdout_head_tail,
            args=(
...
def build_execute_code_schema(enabled_sandbox_tools: set[str] | None = None) -> dict[str, Any]:
    """Build the execute_code schema with description listing only enabled tools.

    When tools are disabled (e.g. web is turned off), the schema
    description should NOT mention web_search / web_extract -- otherwise
    the model thinks they are available and keeps trying to use them.
    """
    ...
    description = (
        "Run a Python script that can call Surogates tools programmatically. "
        ...
        f"Available via `from surogates_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed -- built into surogates_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from surogates_tools import {import_str}` "
...
_DEFAULT_SCHEMA_DICT = build_execute_code_schema()

EXECUTE_CODE_SCHEMA = ToolSchema(
    name="execute_code",
    description=_DEFAULT_SCHEMA_DICT["description"],
    parameters=_DEFAULT_SCHEMA_DICT["parameters"],
)

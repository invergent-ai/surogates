...
The row is named per task (``eog-<task_id>``) and removed right
...
Auth is an ops user session: pass a ready JWT via ``EOG_OPS_TOKEN``,
or a ``EOG_OPS_USER`` / ``EOG_OPS_PASSWORD`` pair and the
...
password at all — those must use ``EOG_OPS_TOKEN``.
...
class Registrar:
        ...
        *,
        ...
        if not token and not (username and password):
            raise RegistrarError(
                "ops auth missing: set EOG_OPS_TOKEN, or "
                "EOG_OPS_USER and EOG_OPS_PASSWORD"
            )
    ...
    def _account(self) -> dict[str, Any]:
        ...
        if resp.status_code == 404:
            raise RegistrarError(
                ...
                f"(EOG_OPS_USER takes the dashboard username or email)"
            )
        ...
        resp = self._check(
            self._request(
                ...
                json={
                    ...
                    "description": "EnterpriseOps-Gym gym server (transient, per-task)",
                    "transport": "http",
                    "url": url,
                    "timeout": timeout,
                    "enabled": True,
    ...
    def cleanup_all(self) -> int:
        """Remove every eog-* row (crash residue); returns the count."""
        removed = 0
        for row in self._rows():
            if str(row.get("name", "")).startswith("eog-"):
                self._detach_and_delete(row["id"])
                removed += 1
        return removed

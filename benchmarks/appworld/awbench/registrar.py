...
The row is named per task (``aw-<task_id>``) and removed right
...
Auth is an ops user session: pass a ready JWT via ``AW_OPS_TOKEN``,
or a ``AW_OPS_USER`` / ``AW_OPS_PASSWORD`` pair and the
...
password at all — those must use ``AW_OPS_TOKEN``.
...
class Registrar:
        ...
        *,
        ...
        if not token and not (username and password):
            raise RegistrarError(
                "ops auth missing: set AW_OPS_TOKEN, or "
                "AW_OPS_USER and AW_OPS_PASSWORD"
            )
    ...
    def _account(self) -> dict[str, Any]:
        ...
        if resp.status_code == 404:
            raise RegistrarError(
                ...
                f"(AW_OPS_USER takes the dashboard username or email)"
            )
        ...
        resp = self._check(
            self._request(
                ...
                json={
                    ...
                    "description": "AppWorld MCP server (transient, per-task)",
                    "transport": "http",
                    "url": url,
                    "timeout": timeout,
                    "enabled": True,
    ...
    def cleanup_all(self) -> int:
        """Remove every aw-* row (crash residue); returns the count."""
        removed = 0
        for row in self._rows():
            if str(row.get("name", "")).startswith("aw-"):
                self._detach_and_delete(row["id"])
                removed += 1
        return removed

...
Test tool discovery (requires a valid sandbox JWT):
...
print(create_sandbox_token(
    UUID('$ORG_ID'), UUID('$USER_ID'), UUID('$SESSION_ID')
))
")
...
1. **Sandbox pod** calls `POST /mcp/v1/tools/call` with a sandbox JWT and tool name + arguments
2. **Auth** validates the JWT and extracts `org_id`, `user_id`, `session_id`
3. **Loader** merges MCP server configs from platform volume + `mcp_servers` DB table (org-wide + user-specific)
4. **Credential resolver** looks up each `credential_refs` entry in the encrypted `credentials` table, tries user-scoped first then org-scoped
5. **Connection pool** lazily connects to the MCP server (reuses existing connections per tenant), caches tool schemas
6. **Tool call** is forwarded to the MCP server session; the result is sanitized (credentials stripped from error messages) and returned
7. **Idle eviction** closes connections unused for 5 minutes (configurable)
...
| Credentials appearing in error output  | Should not happen — `_sanitize_error()` strips credential patterns. File a bug if it does.        |

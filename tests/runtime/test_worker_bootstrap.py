...
def test_session_org_id_is_resolved_per_session_in_shared_mode():
    """The harness factory rebuilds the TenantContext's asset_root
    from the *session* org in shared mode, never from process-wide
    configured_org_id."""
    src = _read_worker_source()
    ...
    assert "asset_root=f\"{settings.tenant_assets_root}/{session_org_id}\"" in src

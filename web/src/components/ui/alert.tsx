...
function AlertTitle({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        // min-w-0: the grid track is `1fr`, whose items default to
        // min-width:auto — long unbroken content then widens the column past
        // the alert box instead of wrapping inside it.
        "min-w-0 text-sm font-semibold group-has-[>svg]/alert:col-start-2 [&_a]:underline [&_a]:underline-offset-3 [&_a]:hover:text-foreground",
        className
      )}
      {...props}
    />

...
**REQUIRED BACKGROUND:** You MUST understand `test-driven-development` before using this. That skill defines the fundamental RED-GREEN-REFACTOR cycle. This document provides skill-specific test formats (pressure scenarios, rationalization tables).
...
|---------------|----------------------|----------------------------------------------------------|
| **RED**       | Baseline test        | Run scenario WITHOUT skill via `delegate_task`, watch fail |
...
- [ ] **Run WITHOUT the skill** via `delegate_task` — give the sub-agent a realistic task with pressures.
...
**Re-test same scenarios with the updated skill.**
...
**If the agent finds a NEW rationalization:** continue REFACTOR cycle.
**If the agent follows the rule:** success — skill is bulletproof for this scenario.
...
1. **"The skill WAS clear, I chose to ignore it"** — Not a documentation problem. You need a stronger foundational principle. Add "Violating letter is violating spirit".
...
- [ ] Ran scenarios WITHOUT the skill via `delegate_task` (baseline).

...
| Verify token | `vault://whatsapp_verify_token_{pnid}` | generated by us, `secrets.token_urlsafe(32)` |
| WABA id | `channel_routing.config` | Meta App → WhatsApp |
| API version | `channel_routing.config` | default v23.0 |
| Display phone number | `channel_routing.config` + provisioner `extra_env` | Graph-confirmed; see §11 frontend row 2 |
...
`_APP_SECRET`, `_WABA_ID`, `_VERIFY_TOKEN`.
...
| Verify Token | auto-generated, shown once with a copy button; rotation defaults to No |

...
- **Three spawn paths**, one line each, where `worker_config` is already assembled:
  - `tools/builtin/coordinator.py` (`spawn_worker`)
  - `tools/builtin/delegate.py` (`delegate_task`)
  - `tasks/spawn.py::_create_session_for_task` — covers **retry attempts**, which therefore rejoin the same board and inherit prior attempts' notes.
...
1. **Deterministic pre-checks** (free, ordered):
   ...
   3. active-`CLAIM` cap per writer (default 2);
   4. exact-dedupe against the group's *active* notes (normalized content + type). Exception: a `CLAIM` re-posted by its own writer is a **renewal** — refresh `expires_at`, bump `seq`, skip insert.
...
- **Join snapshot**: on a member session's first wake with the group key (no cursor yet), append one `board.update` event containing the full windowed render; set `session.config["board_cursor"] = max(seq)`.
...
- **Purge**: cleanup-job extension deletes notes whose group root session has been terminal (`done`/`failed`) for > `board_purge_after_days` (default 7).
...
- **Unit**: pre-check ordering and rejections (caps, types, injection/secret scan, claim cap, dedupe, claim renewal); verifier admission with mocked LLM (keep/reject parsing, fail-closed on error/timeout/garbage); windowed render (priority order, FAIL reserve, expiry filtering, supersede, overflow line); delta rendering of status transitions; cursor advancement rules.

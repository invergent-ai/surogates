...
//
// The composer's builtin slash menu gates the ``/auto-research`` entry
// on the ``researchEnabled`` prop. Studio reads the flag from the agent
// record and passes it through; without the flag the command would run
// against a harness that has no arbor-executor sub-agent in the bundle.
...
describe("Composer builtin /auto-research suggestion gating", () => {
  const sendFn = () => Promise.resolve();
  const stopFn = () => Promise.resolve();

  it("omits /auto-research from the menu by default", () => {
    const dom = mount(
      <ChatComposer
        onSend={sendFn}
        onStop={stopFn}
    ...
    expect(labels.some((l) => l.includes("/auto-research"))).toBe(false);
    expect(labels.some((l) => l.includes("/clear"))).toBe(true);
  });

  it("shows /auto-research when researchEnabled is true", () => {
    const dom = mount(
      <ChatComposer
        onSend={sendFn}
        onStop={stopFn}
    ...
    openCommandsMenu(dom);
    const labels = menuItems();
    expect(labels.some((l) => l.includes("/auto-research"))).toBe(true);
  });
});
